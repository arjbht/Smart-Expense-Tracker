package com.arjun.expensetracker.di

import android.content.Context
import androidx.room.Room
import com.arjun.expensetracker.data.room.AppDatabase
import com.arjun.expensetracker.repository.ExpenseRepository
import com.arjun.expensetracker.repository.RoomExpenseRepository
import dagger.Module
import dagger.Provides
import dagger.hilt.InstallIn
import dagger.hilt.android.qualifiers.ApplicationContext
import dagger.hilt.components.SingletonComponent
import javax.inject.Singleton


@Module
@InstallIn(SingletonComponent::class)
object AppModule {

    @Provides
    @Singleton
    fun provideDatabase(@ApplicationContext context: Context): AppDatabase =
        Room.databaseBuilder(context, AppDatabase::class.java, "expenses.db").build()

    @Provides
    @Singleton
    fun provideExpenseRepository(db: AppDatabase): ExpenseRepository =
        RoomExpenseRepository(db.expenseDao())
}